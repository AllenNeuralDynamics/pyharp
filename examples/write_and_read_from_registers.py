...
#analog_threshold_h = device.send(HarpMessage.ReadU16(42)).payload_as_int()
...
data_stream = device.send(HarpMessage.ReadU8(33)) # returns a ReplyHarpMessage
#data_stream = device.send(HarpMessage.ReadS16(33)).payload_as_int_array()
...
print(f"System time: {time.perf_counter():.6f}")
event_reg_response = device.send(HarpMessage.ReadU8(77)) # returns a ReplyHarpMessage
...
#device.send(HarpMessage.WriteU16(42, analog_threshold_h+1))
#
...
#analog_sensor = []
#for x in range(10):
#    value = device.send(HarpMessage.ReadS16(33)).payload_as_int()
